...
var util = require('util'),
	...
	SoundwebSerialFileParser = require('./soundweb_serial_file_parser.js').Parser,
	net = require('net');
...
SoundwebClient.prototype.connect = function() {
	var self = this;
	...
	this._decoder.on('INVALID_CHECKSUM', function(receivedChecksum, calculatedChecksum, bodyBytes) {
		self._send(new Buffer([SoundwebConsts.special.byStr.NAK]));
		console.log("Checksum Invalid", receivedChecksum, calculatedChecksum, bodyBytes);
	});
...
SoundwebClient.prototype._set_value = function(msg) {
	...
	this.emit('SET_VALUE', SoundwebConsts.groups.byVal[msg[1]], msg[2], msg.readUInt16BE(3));
};
...
exports.createParser = function(serial_file) {
	return new SoundwebSerialFileParser(serial_file);
};
